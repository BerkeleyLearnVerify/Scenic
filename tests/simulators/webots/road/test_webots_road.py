...
def test_basic(loadLocalScenario):
    scenario = loadLocalScenario('crossing.scenic', mode_2d=True)
    scenario.generate(maxIterations=1000)

def test_road_only(loadLocalScenario):
    scenario = loadLocalScenario('simple.scenic', mode_2d=True)
...
def test_curb(loadLocalScenario):
    scenario = loadLocalScenario('curb.scenic', mode_2d=True)
...
def test_noninterference(runLocally):
    ...
    with runLocally():
        scenario = compileScenic(code.format(world='richmond.wbt'), mode_2d=True)
        assert len(scenario.objects) == 1
        roads1 = scenario.workspace.roads
        assert len(roads1) > 1
        scenario = compileScenic(code.format(world='simple.wbt'), mode_2d=True)
        assert len(scenario.objects) == 1
