...
WEBOTS_WORLD_FILE_PATH=f"{os.path.dirname(__file__)}/dynamic/webots_data/worlds/world.wbt"
...
def cleanup_results():
    ...
    if os.path.exists(WEBOTS_RESULTS_FILE_PATH):
        assert False, f"Clean up of {WEBOTS_RESULTS_FILE_PATH} for webots tests failed!"
    return 

def test_dynamics_scenarios(webotsAvailable):
    webotsAvailable(WEBOTS_BINARY_PATH)
    cleanup_results()
    command = f"bash {WEBOTS_BINARY_PATH} --no-rendering --minimize {WEBOTS_WORLD_FILE_PATH}"
    _ = subprocess.run(command, shell=True)
