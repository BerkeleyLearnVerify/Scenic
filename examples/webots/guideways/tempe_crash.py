# Hack to synchronize guideways and Webots coordinates for Uber Crash experiments
...
with open(osmPath, "r") as osm:
    for line in osm:
        if "bounds" in line:
            minlat = float(
                re.findall(r"[-+]?\d*\.\d+|\d+", line[line.find("minlat") :])[0]
            )
            minlon = float(
                re.findall(r"[-+]?\d*\.\d+|\d+", line[line.find("minlon") :])[0]
            )
            maxlat = float(
                re.findall(r"[-+]?\d*\.\d+|\d+", line[line.find("maxlat") :])[0]
            )
            maxlon = float(
                re.findall(r"[-+]?\d*\.\d+|\d+", line[line.find("maxlon") :])[0]
            )
