To run the CoSimulator you first must start by opening and running both CARLA and METS-R 
...
Finally to run the scenic program first ensure
    (i) The `globalParameter address` set at the top of your Scenic program matches your current IP address to allow Scenic to connect to CARLA
    (2) The `globalParameter map` set at the top of your Scenic program provides the path to the corresponding `xodr` map you would like to run
        (a) Ensure that this parameter matches the configuration file used to spin up METSR
    (3) The `globalParameter xml_map` matches provides the path to the corresponding `xml` map. 
