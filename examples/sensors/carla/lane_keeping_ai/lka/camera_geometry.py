...
def project_polyline(polyline_world, trafo_world_to_cam, K):
    ...
    depth = pl_uv_cam[:, 2]
    return (np.stack((u, v)).T).astype(np.int32), depth


class CameraGeometry(object):
    def __init__(self, offset=(0.5, 0, 1.3), rotation=(-5, 0, 0), image_width=1024, image_height=512,
                 field_of_view_deg=45):
        # scalar constants
        self.height = offset[2]
        self.displacement = offset[0]
        self.pitch_deg = rotation[0]
        self.roll_deg = rotation[1]
        self.yaw_deg = rotation[2]
