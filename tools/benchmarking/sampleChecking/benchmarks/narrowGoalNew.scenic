...
class BigRock(Debris):
    """Large rock."""
    shape: MeshShape.fromFile(Path(localPath(".")).parent.parent.parent.parent / "tools" / "meshes" / "webots_rock_large.obj.bz2")
    yaw: Range(0, 360 deg)
    webotsType: 'ROCK_BIG'
...
class Rock(Debris):
    """Small rock."""
    shape: MeshShape.fromFile(Path(localPath(".")).parent.parent.parent.parent / "tools" / "meshes" / "webots_rock_small.obj.bz2")
    yaw: Range(0, 360 deg)
