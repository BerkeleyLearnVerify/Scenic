...
def GetObjectPose(obj, frame="map"):  # works
    """
    Returns the pose of the object in Gazebo's map frame (not the HSR map frame)
...
def GetObjectState(obj, frame="map"):
    ...
    Returns the state of the object in Gazebo's map frame (not the HSR map frame)
    String obj: the name of the object
    String frame: the reference frame for the pose
...
def GetHSRWorldProperties():  # Good
    """
    Get the world properties of the current simulation

    return: GetWorldProperties return value:
    ...
    """
    # TODO apply further check for world_property.success being True
    try:
...
def GetHSRWorldModelNames():  # works
    world_properties = GetHSRWorldProperties()
    if world_properties:
        return world_properties.model_names
    # rospy.logerr('Fail go') # TODO uncomment this later when done with initial coding
    return None
