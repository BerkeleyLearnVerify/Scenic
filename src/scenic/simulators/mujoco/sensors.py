...
class MujocoRGBSensor(RGBSensor):
    ...
    def __init__(self, offset=(0,0,0), rotation=(0, 0, 0), width=None, height=None, attributes=None):
    ...
    def getObservation(self):
        ...
        if (hasattr(self.renderer, '_shared_rgb_time') and 
            hasattr(self.renderer, '_shared_rgb_data') and
            self.renderer._shared_rgb_time == current_time and
...
class MujocoSSSensor(SSSensor):
    ...
    def __init__(self, offset=None, rotation=(0, 0, 0), width=None, height=None, 
                 attributes=None, use_object_types=False, color_map=None, color_by_instance=True):
        if width is None or height is None:
            raise ValueError("width and height are required for sensors")
        ...
        self.color_by_instance = color_by_instance  # Color whole Scenic objects vs individual geoms.
    ...
    def getObservation(self):
        ...
        if current_time == self._last_render_time and hasattr(self, '_cached_segmentation'):
        ...
        try:
            ...
            if not hasattr(self.renderer, '_seg_renderer'):
                self.renderer._seg_renderer = mujoco.Renderer(self.model, height=self.height, width=self.width)
            ...
            if 0 <= geom_id < self.model.ngeom:
                ...
                if geom_name:
                    ...
                    if scenic_obj:
                        ...
                        if hasattr(scenic_obj, 'semanticColorForGeom') and callable(scenic_obj.semanticColorForGeom):
                            try:
                                geom_color = scenic_obj.semanticColorForGeom(geom_name)
                                if geom_color is not None:
                                    if isinstance(geom_color, (list, tuple)) and len(geom_color) == 3:
                                        return list(geom_color)
                            except Exception as e:
                                print(f"Warning: semanticColorForGeom failed for {geom_name}: {e}")
                        
                        # Priority 2: Use object's semanticColor property.
                        if hasattr(scenic_obj, 'semanticColor') and scenic_obj.semanticColor is not None:
                    ...
                    if class_name:
                        ...
                        if class_name not in self._class_color_map:
                            ...
                            if self.color_by_instance:
                                ...
                                self._class_color_map[class_name] = self._color_palette[color_idx]
