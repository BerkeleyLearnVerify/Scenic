...
class Guideway(Bordered):
    def __init__(self, proj, data):
        ...
        self.region.orientation = VectorField(
            f"Road{self.id}Direction", self.directionAt
        )
...
def projectionAt(point):
    ...
    string = (
        "+proj=ortho +%s +zone=%d +lon_0=%f +lat_0=%f +x_0=0 +y_0=0 +ellps=WGS84 +units=m +no_defs"
        % (hemisphere, utm_zone, lon, lat)
    )
...
class Intersection:
    def __init__(self, data, origin=None):
        ...
        self.vehicleGuideways = tuple(
            Guideway(self.projection, d) for d in data["vehicle_gw"]
        ) + (self.mainGuideway,)
        self.bikeGuideways = tuple(
            Guideway(self.projection, d) for d in data["bicycle_gw"]
        )
        self.railGuideways = tuple(
            Guideway(self.projection, d) for d in data["rail_gw"]
        )
        guidewayWithID = {}
        for gw in itertools.chain(
            self.vehicleGuideways, self.bikeGuideways, self.railGuideways
        ):
            guidewayWithID[gw.id] = gw
        self.guidewayWithID = guidewayWithID

        self.crosswalks = tuple(
            Crosswalk(self.projection, d) for d in data["crosswalks"]
        )
        self.conflictZones = tuple(
            ConflictZone(self, d) for d in data["conflict_zones"]
        )
...
class IntersectionWorkspace(Workspace):
    def __init__(self, intersection):
        ...
        drivablePoly = polygonUnion(
            gw.region.polygons for gw in intersection.vehicleGuideways
        )
        walkablePoly = polygonUnion(
            cw.region.polygons for cw in intersection.crosswalks
        )
