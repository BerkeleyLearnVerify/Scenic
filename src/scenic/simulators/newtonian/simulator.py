...
from scenic.domains.driving.simulators import DrivingSimulator, DrivingSimulation, DrivingGymSimulation
...
class NewtonianSimulator(DrivingSimulator):
    ...
    def createGymSimulation(self, scene, verbosity=0):
        return NewtonianGymSimulation(scene, self.network, timestep=self.timestep,
                                   verbosity=verbosity, render=self.render)

class NewtonianGymSimulation(DrivingGymSimulation):
    ...
    def checkCollision(self):
        for obj in self.objects:
            for other_obj in self.objects:
                if self.compute_distance(obj.position,
                                         other_obj.position) < 4:
                    return True
        return False

    def computeReward(self):
        # TODO: Make a better reward (placeholder)
    ...
    def simulator_step(self, action):
        for obj in self.objects:
            if obj is self.ego and action is not None:
                acceleration, obj.steer, obj.brake = action
            if obj.hand_brake:
    ...
    def currentState(self):
        ...
        In the newtonian simulator, have this be the properties for each object
        """
        return tuple(self.getProperties(obj) for obj in self.objects)
    ...
    def getProperties(self, obj, properties=None):
        values = dict(
