...
def compute_gradient_manual(img_data=None, img_file=None, threshold=100, kernelsize=1):
    assert img_data is not None or img_file is not None
    if img_data is None:
        img_data = Image.open(img_file)
...
def compute_midpoints(
    ...
    for edge in connected_edges:
        for x in edge:
            ...
            point_data[x] = EdgeData(
                init_theta=init_theta,
                tangent=theta,
                opp_loc=opp_loc,
                mid_loc=mid_loc if opp_loc not in bb else (np.nan, np.nan),
            )
...
def compute_heading(x, point_data):
    ...
    edge_distance = np.linalg.norm(np.array(list(road_edges)) - np.array(x), 1, axis=1)
