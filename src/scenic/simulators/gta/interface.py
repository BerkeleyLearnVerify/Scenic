...
class Map:
    ...
    def __init__(self, imagePath, Ax, Ay, Bx, By):
        ...
        if imagePath != None:
            ...
            self.edgeData = {
                self.gridToScenicCoords((x, y)): datum for (y, x), datum in ed.items()
            }
    ...
    def fromFile(path):
        ...
        with numpy.load(path, allow_pickle=True) as data:
            ...
            m.edgeData = {
                tuple(e): center_detection.EdgeData(*rest) for e, *rest in data["edges"]
            }
    ...
    def dumpToFile(self, path):
        ...
        edges = numpy.array(
            [(edge,) + tuple(datum) for edge, datum in self.edgeData.items()]
        )
    ...
    @utils.cached
    def curbRegion(self):
        return PointSetRegion(
            "curb",
            self.orderedCurbPoints,
