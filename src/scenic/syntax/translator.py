...
def scenarioFromStream(
    ...
    try:
        with topLevelNamespace(path) as namespace:
            compileStream(
                stream, namespace, params=params, model=model, filename=filename
            )
    finally:
        if not cacheImports:
            toRemove = []
            for name, module in sys.modules.items():
                ...
                elif name.startswith(("scenic.domains.", "scenic.simulators.")) and any(
                    getattr(val, "_isScenicModule", False)
...
def compileStream(stream, namespace, params={}, model=None, filename="<stream>"):
    ...
    try:
        ...
        raise TokenParseError(
            line, filename, "file ended during multiline string or expression"
        )
    ...
    try:
        ...
        for blockNum, block in enumerate(blocks):
            ...
            trimmed = newSource[
                2 * (startLine - 1) :
            ]  # fix up blank lines used to align errors
            newSource = "\n" * (startLine - 1) + trimmed
            newSourceBlocks.append(trimmed)
            if dumpTranslatedPython:
                print(
                    f"### Begin translated Python from block {blockNum} of {filename}"
                )
                print(newSource)
            ...
            if dumpASTPython:
                ...
                print(
                    f"### Begin Python equivalent of final AST from block {blockNum} of {filename}"
                )
...
softRequirement = (
    "require_soft"  # not actually a statement, but a marker for the parser
)
...
twoWordStatements = {
    requireAlwaysStatement,
    requireEventuallyStatement,
    terminateWhenStatement,
    terminateAfterStatement,
    recordInitialStatement,
    recordFinalStatement,
    *invokeVariants,
}
threeWordStatements = {terminateSimulationWhenStatement}

threeWordIncipits = {
    tokens[:2]: tokens[2] for tokens in threeWordStatements
}  # TODO improve
...
recordStatements = (
    recordStatement,
    functionForStatement(recordInitialStatement),
...
for op in infixOperators:
    ...
    if imp is not None:
        ...
        node = op.node
        if (
            node in infixImplementations
        ):  # two operators may have the same implementation
...
generalInfixOps = {
    tokens: op.token for tokens, op in infixTokens.items() if not op.contexts
}
...
keywords = set(constructorStatements) | internalFunctions | replacements.keys()
            ...
            for extension in scenicExtensions:
                ...
                if os.path.exists(filepath):
                    ...
                    spec = importlib.util.spec_from_file_location(
                        name, filepath, loader=ScenicLoader(filepath, filename)
                    )
                    return spec
        return None
    ...
    def exec_module(self, module):
        ...
        with open(self.filepath, "rb") as stream:
            code, pythonSource = compileStream(
                stream, module.__dict__, filename=self.filepath
            )
    ...
    def translate(self, tokens):
        ...
        for token in tokens:
            ...
            def popFunction():
                ...
                injectToken((RPAR, ")"))
                context, startLevel = (
                    (None, 0) if len(functionStack) == 0 else functionStack[-1]
                )
            ...
            if ttype == LPAR or ttype == LSQB:  # keep track of nesting level
                ...
                if (
                    len(functionStack) > 0
                    and functionStack[-1][0] == paramStatement
                ...
                ttype == NAME
            ):  # the interesting case: almost all new syntax falls in here
                # try to match 2-word language constructs
                matched = False
                nextToken = peek(
                    tokens
                )  # lookahead so we can give 2-word ops precedence
                if nextToken is not None:
                    ...
                    if startOfLine and tstring == "for":  # TODO improve hack?
                        ...
                        startOfLine and tstring in constructorStatements
                    ):  # class definition
                        if nextToken.type != NAME or nextString in keywords:
                            self.parseError(
                                nextToken, f'invalid class name "{nextString}"'
                            )
                        ...
                        if peek(tokens).exact_type == LPAR:  # superclass specification
                            ...
                            while nextToken.exact_type != RPAR:
                                ...
                                if nextToken.exact_type != NAME:
                                    self.parseError(
                                        nextToken, f'invalid superclass "{base}"'
                                    )
                                bases.append(base)
                        ...
                        if nextToken.type != NAME:
                            self.parseError(
                                nextToken, f'invalid scenario name "{nextString}"'
                            )
                        ...
                        startOfLine and tstring == behaviorStatement
                    ):  # behavior definition
                        if nextToken.type != NAME:
                            self.parseError(
                                nextToken, f'invalid behavior name "{nextString}"'
                            )
                        injectToken((NAME, "async"), spaceAfter=1)
                        injectToken((NAME, "def"), spaceAfter=1)
                        skip = True
                        ...
                        startOfLine and tstring == monitorStatement
                    ):  # monitor definition
                        if nextToken.type != NAME:
                            self.parseError(
                                nextToken, f'invalid monitor name "{nextString}"'
                            )
                        injectToken((NAME, "async"), spaceAfter=1)
                        injectToken((NAME, "def"), spaceAfter=1)
                        ...
                        not startOfStatement and twoWords in allowedInfixOps
                    ):  # 2-word infix operator
                        ...
                        startOfStatement and twoWords in threeWordIncipits
                    ):  # 3-word statement
                        ...
                        if thirdWord != expected:  # TODO do proper 3-word lookahead?
                            self.parseError(
                                endToken, f'expected "{expected}", got "{thirdWord}"'
                            )
                        wrapStatementCall()
                        ...
                        startOfStatement and twoWords in twoWordStatements
                    ):  # 2-word statement
                        wrapStatementCall()
                        ...
                        inConstructorContext and tstring == "with"
                    ):  # special case for 'with' specifier
                        callFunction("With", argument=(STRING, f'"{nextString}"'))
                        advance()  # consume property name
                    elif (
                        startOfStatement
                        and tstring == requireStatement
                        and nextString == "["
                    ):
                        ...
                        if not 0 <= float(prob) <= 1:
                            self.parseError(
                                nextToken, "probability must be between 0 and 1"
                            )
                        nextToken = next(tokens)
                ...
                if not matched:
                    ...
                    if oneWord in allowedPrefixOps:  # 1-word prefix operator
                        ...
                        startOfStatement and tstring in oneWordStatements
                    ):  # 1-word statement
                        wrapStatementCall()
                        callFunction(tstring)
                    elif (
                        token.start[1] == 0 and tstring == modelStatement
                    ):  # model statement
                        components = []
                        ...
                        if not components:
                            self.parseError(
                                token, "model statement is missing module name"
                            )
                        components.append("'")
                        literal = "'" + "".join(components)
                        callFunction(
                            modelStatement, argument=(NAME, namespaceReference)
                        )
                        injectToken((STRING, literal))
                        skip = True
                    elif (
                        startOfLine and tstring in scenarioBlocks
                    ):  # named block of scenario
                        if peek(tokens).exact_type != COLON:
                            self.parseError(
                                peek(tokens), f'malformed "{tstring}" block'
                            )
                        injectToken((NAME, "async"), spaceAfter=1)
                        injectToken((NAME, "def"), spaceAfter=1)
                        ...
                        injectToken((RPAR, ")"))
                        skip = True
                    elif tstring in self.constructors and peek(
                        tokens
                    ).exact_type not in (RPAR, RSQB, RBRACE, COMMA, DOT):
                        # instance definition
                        callFunction(tstring)
                        ...
                        startOfLine and tstring == "from"
                    ):  # special case to allow 'from X import Y'
                        pass
                        ...
                        self.parseError(
                            token, f'Python construct "{tstring}" not allowed in Scenic'
                        )
                    else:
            ...
            if len(functionStack) > 0:
                ...
                while (
                    parenLevel < startLevel
                ):  # we've closed all parens for the current function
                    popFunction()
                inConstructor = any(
                    context in constructors for context, sl in functionStack
                )
                if (
                    inConstructor and parenLevel == startLevel and ttype == COMMA
                ):  # starting a new specifier
                    ...
                    while nextToken.exact_type in (NEWLINE, NL, COMMENT, ENDMARKER):
                        ...
                        if nextToken.exact_type not in (NEWLINE, NL):
                            self.parseError(
                                nextToken, "comma with no specifier following"
                            )
...
tempArg = ast.arguments(
    posonlyargs=[],
    args=[ast.arg(arg=temporaryName, annotation=None)],
    vararg=None,
    kwonlyargs=[],
    kw_defaults=[],
    kwarg=None,
...
class ASTSurgeon(NodeTransformer):
    ...
    def unpack(self, arg, expected, node):
        ...
        if isinstance(arg, BinOp) and isinstance(arg.op, packageNode):
            if expected == 1:
                ...
                return self.unpack(arg.left, expected - 1, node) + [
                    self.visit(arg.right)
                ]
        elif expected > 1:
    ...
    def visit_Raise(self, node):
        ...
        if self.inCompose and func.id not in compositionalImps:
            ...
            self.parseError(
                node, f'"{statement}" cannot be used in a {composeBlock} block'
            )
        ...
        if func.id in requirementStatements:  # require, terminate when, etc.
            ...
            if (
                func.id == softRequirement
            ):  # extract probability as first arg for soft reqs
            ...
            self.requirements.append(
                req
            )  # save condition for later inspection when pruning
        ...
        elif func.id == invokeStatement or func.id.startswith(invokeStatement + "_"):
            ...
            for arg in node.args:
                if (
                    ...
                    if seenModifier:
                        self.parseError(
                            arg, f'incompatible qualifiers for "{stmt}" statement'
                        )
                    seenModifier = True
            ...
            self.validateSimpleCall(
                node, (1, maxInvoked), onlyInBehaviors=True, args=invoked
            )
            subHandler = Attribute(
                Name(behaviorArgName, Load()), "_invokeSubBehavior", Load()
            )
            ...
            if self.inCompose:
                self.parseError(
                    func, f'cannot use "{actionStatement}" in a {composeBlock} block'
                )
            ...
            self.validateSimpleCall(node, 0, onlyInBehaviors=True)
            termination = Call(
                Name(createTerminationAction, Load()), [Constant(node.lineno)], []
            )
    ...
    def validateSimpleCall(self, node, numArgs, onlyInBehaviors=False, args=None):
        ...
        for arg in args:
            if isinstance(arg, Starred):
                self.parseError(
                    node, f'argument unpacking cannot be used with "{name}"'
                )
    ...
    def generateInvocation(self, node, actionlike, invoker=Yield):
        ...
        args = Starred(
            Attribute(Name(behaviorArgName, Load()), "_args", Load()), Load()
        )
        kwargs = ast.keyword(
            None, Attribute(Name(behaviorArgName, Load()), "_kwargs", Load())
        )
    ...
    def visit_Try(self, node):
        ...
        for handler in node.handlers:
            ...
            if (
                isinstance(ty, Call)
                and isinstance(ty.func, Name)
                and ty.func.id == interruptExceptMarker
            ):
                assert handler.name is None
                if len(ty.args) != 1:
                    self.parseError(
                        handler, '"interrupt when" takes a single expression'
                    )
    ...
    def visit_Call(self, node):
        ...
        if wrappedStar:
            newNode = Call(
                Name("callWithStarArgs", Load()), [newFunc] + newArgs, newKeywords
            )
        else:
    ...
    def transformScenarioDefinition(self, node):
        if self.inCompose:
            self.parseError(
                node, f"cannot define a scenario inside a {composeBlock} block"
            )
        if self.inBehavior:
            ...
            if isinstance(statement, AsyncFunctionDef):
                if statement.name == setupBlock:
                    ...
                    if len(statement.body) != 1 or not isinstance(
                        statement.body[0], Expr
                    ):
                        self.parseError(
                            statement.body[0], f"malformed precondition/invariant"
                        )
                    self.inGuard = True
        ...
        if simple:  # simple scenario: entire body is implicitly the setup block
            if setup:
                self.parseError(
                    setup, f'simple scenario cannot have a "{setupBlock}" block'
                )
            if compose:
                self.parseError(
                    compose, f'simple scenario cannot have a "{composeBlock}" block'
                )
            if preconditions or invariants:
                self.parseError(
                    node, f"simple scenario cannot have preconditions/invariants"
                )
        ...
        newDef = FunctionDef(
            "_setup", args, newBody, [Name(id="staticmethod", ctx=Load())], None
        )
    ...
    def transformBehaviorDefinition(self, node):
        if self.inCompose:
            self.parseError(
                node, f"cannot define a behavior inside a {composeBlock} block"
            )
        if self.inBehavior:
        ...
        for statement in oldStatements:
            if isinstance(statement, AsyncFunctionDef):
                ...
                if len(statement.body) != 1 or not isinstance(statement.body[0], Expr):
                    self.parseError(
                        statement.body, f"malformed behavior {statement.name}"
                    )
                self.inGuard = True
                test = self.visit(statement.body[0].value)
                self.inGuard = False
        ...
        genDefn = FunctionDef(
            "makeGenerator", newArgs, newBody, decorators, node.returns
        )
    ...
    def makeGuardCheckers(self, args, preconditions, invariants):
        ...
        for precondition in preconditions:
            call = Call(
                Name("PreconditionViolation", Load()),
        ...
        definePChecker = FunctionDef(
            checkPreconditionsName, args, precondChecks + [ast.Pass()], [], None
        )
        # generate invariant checker
        invChecks = []
        for invariant in invariants:
            call = Call(
                Name("InvariantViolation", Load()),
        ...
        defineIChecker = FunctionDef(
            checkInvariantsName, args, invChecks + [ast.Pass()], [], None
        )
    ...
    def visit_Yield(self, node):
        if self.inCompose:
            self.parseError(
                node, f'"yield" is not allowed inside a {composeBlock} block'
            )
        if self.inBehavior:
    ...
    def visit_YieldFrom(self, node):
        if self.inCompose:
            self.parseError(
                node, f'"yield from" is not allowed inside a {composeBlock} block'
            )
        if self.inBehavior:
                    ...
                    if isinstance(sl, Name):
                        ...
                        for elt in sl.elts:
                            if not isinstance(elt, Name):
                                self.parseError(
                                    elt, "malformed attributes for property default"
                                )
...
def gatherBehaviorNamespacesFrom(behaviors):
    ...
    def registerNamespace(modName, ns):
        ...
        for name, value in ns.items():
            if isinstance(value, types.ModuleType) and getattr(
                value, "_isScenicModule", False
            ):
