...
def record_initial(reqID, value, line, name):
    if not name:
        name = f"record{line}"
    makeRequirement(
        requirements.RequirementType.recordInitial, reqID, value, line, name
    )
...
def RelativeTo(X, Y):
    ...
    if xf or yf:
        ...
        if isinstance(X, OrientedPoint):  # TODO too strict?
            ...
            Y = toVector(
                Y, '"X relative to Y" with X an oriented point but Y not a vector'
            )
            return X.relativize(Y)
        elif isinstance(Y, OrientedPoint):
            X = toVector(
                X, '"X relative to Y" with Y an oriented point but X not a vector'
            )
            ...
            return evaluateRequiringEqualTypes(
                lambda: X + Y, X, Y, '"X relative to Y" with vector and scalar'
            )
...
def Facing(heading):
    ...
    if isinstance(heading, VectorField):
        return Specifier(
            "heading",
            DelayedArgument({"position"}, lambda self: heading[self.position]),
        )
    else:
        heading = toHeading(
            heading, 'specifier "facing X" with X not a heading or vector field'
        )
...
def FacingToward(pos):
    ...
    return Specifier(
        "heading",
        DelayedArgument({"position"}, lambda self: self.position.angleTo(pos)),
    )
