...
class CustomDistribution(Distribution):
    ...
    def __init__(
        self, sampler, *dependencies, name="CustomDistribution", evaluator=None
    ):
        super().__init__(*dependencies)
...
class TupleDistribution(Distribution, collections.abc.Sequence):
    ...
    def isEquivalentTo(self, other):
        if not type(other) is TupleDistribution:
            return False
        return (
            areEquivalent(self.coordinates, other.coordinates)
...
class FunctionDistribution(Distribution):
    ...
    def evaluateInner(self, context):
        ...
        kwargs = {
            name: valueInContext(arg, context) for name, arg in self.kwargs.items()
        }
...
class AttributeDistribution(Distribution):
    ...
    def isEquivalentTo(self, other):
        if not type(other) is AttributeDistribution:
            return False
        return self.attribute == other.attribute and areEquivalent(
...
class MultiplexerDistribution(Distribution):
    ...
    def isEquivalentTo(self, other):
        if not type(other) == type(self):
            return False
        return areEquivalent(self.index, other.index) and areEquivalent(
...
class Range(Distribution):
    ...
    def isEquivalentTo(self, other):
        if not type(other) is Range:
            return False
        return areEquivalent(self.low, other.low) and areEquivalent(
...
class Normal(Distribution):
    ...
    def isEquivalentTo(self, other):
        if not type(other) is Normal:
            return False
        return areEquivalent(self.mean, other.mean) and areEquivalent(
...
class TruncatedNormal(Normal):
    ...
    def bucket(self, buckets=None):
        ...
        if isinstance(buckets, int):
            if buckets < 1:
                raise RuntimeError(
                    f"Invalid buckets for TruncatedNormal.bucket: {buckets}"
                )
            endpoints = numpy.linspace(self.low, self.high, buckets + 1)
        else:
            endpoints = tuple(buckets)
    ...
    def isEquivalentTo(self, other):
        if not type(other) is TruncatedNormal:
            return False
        return (
            areEquivalent(self.mean, other.mean)
...
class DiscreteRange(Distribution):
    ...
    def __init__(self, low, high, weights=None):
        if not isinstance(low, int):
            raise RuntimeError(
                f"DiscreteRange endpoint {low} is not a constant integer"
            )
        if not isinstance(high, int):
            raise RuntimeError(
                f"DiscreteRange endpoint {high} is not a constant integer"
            )
        if not low <= high:
            raise RuntimeError(
