...
class Scenario:
    ...
    def __init__(
        ...
        self.dependencies = (
            self.objects + paramDeps + tuple(requirementDeps) + tuple(behaviorDeps)
        )
    ...
    def isEquivalentTo(self, other):
        if type(other) is not Scenario:
            return False
        return (
            areEquivalent(other.workspace, self.workspace)
    ...
    def generate(self, maxIterations=2000, verbosity=0, feedback=None):
        ...
        activeReqs = [
            req for req in self.initialRequirements if random.random() <= req.prob
        ]
        ...
        eventuallyReqs = (
            BoundRequirement(req, sample) for req in self.eventuallyRequirements
        )
        terminationConds = (
            BoundRequirement(req, sample) for req in self.terminationConditions
        )
        termSimulationConds = (
            BoundRequirement(req, sample) for req in self.terminateSimulationConditions
        )
        recordedExprs = (BoundRequirement(req, sample) for req in self.recordedExprs)
        recordedInitialExprs = (
            BoundRequirement(req, sample) for req in self.recordedInitialExprs
        )
        recordedFinalExprs = (
            BoundRequirement(req, sample) for req in self.recordedFinalExprs
        )
        scene = Scene(
            self.workspace,
            sampledObjects,
