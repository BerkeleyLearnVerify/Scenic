...
def regionFromShapelyObject(obj, orientation=None):
    ...
    if obj.is_empty:
        ...
        obj, (shapely.geometry.LineString, shapely.geometry.MultiLineString)
    ):
...
class CircularRegion(Region):
    ...
    def evaluateInner(self, context):
        ...
        return CircularRegion(
            center, radius, name=self.name, resolution=self.resolution
        )
    ...
    def isEquivalentTo(self, other):
        if type(other) is not CircularRegion:
            return False
        return areEquivalent(other.center, self.center) and areEquivalent(
...
class SectorRegion(Region):
    ...
    def containsPoint(self, point):
        point = point.toVector()
        if not pointIsInCone(
            tuple(point), tuple(self.center), self.heading, self.angle
        ):
            return False
        return point.distanceTo(self.center) <= self.radius
    ...
    def isEquivalentTo(self, other):
        if type(other) is not SectorRegion:
            return False
        return (
            areEquivalent(other.center, self.center)
...
class RectangularRegion(_RotatedRectangle, Region):
    ...
    def isEquivalentTo(self, other):
        if type(other) is not RectangularRegion:
            return False
        return (
            areEquivalent(other.position, self.position)
            and areEquivalent(other.heading, self.heading)
...
class PolylineRegion(Region):
    ...
    def difference(self, other):
        ...
        if poly is not None:
            ...
            if diff.is_empty or not isinstance(
                diff, (shapely.geometry.LineString, shapely.geometry.MultiLineString)
            ):
                # TODO handle points!
                return nowhere
    ...
    def project(self, point):
        return shapely.ops.nearest_points(
            self.lineString, shapely.geometry.Point(point)
        )[0]
...
class PolygonalRegion(Region):
    ...
    def __init__(self, points=None, polygon=None, orientation=None, name=None):
        ...
        if polygon is None:
            ...
            if len(points) == 0:
                raise RuntimeError(
                    "tried to create PolygonalRegion from empty point list!"
                )
            for point in points:
        ...
        if not self.polygons.is_valid:
            raise RuntimeError(
                "tried to create PolygonalRegion with "
    ...
    def difference(self, other):
        ...
        if poly is not None:
            ...
            if diff.is_empty:
                ...
                diff, (shapely.geometry.Polygon, shapely.geometry.MultiPolygon)
            ):
    ...
    def intersect(self, other, triedReversed=False):
        poly = toPolygon(other)
        orientation = (
            other.orientation if self.orientation is None else self.orientation
        )
        if poly is not None:
        ...
        def sampler(intRegion):
            ...
            if len(intersection) == 0:
                raise RejectionException(
                    f"empty intersection of Regions {self} and {o}"
                )
            return self.orient(random.choice(intersection))

        return IntersectionRegion(
            self, other, sampler=sampler, orientation=self.orientation
        )
