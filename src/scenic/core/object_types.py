...
from scenic.core.geometry import averageVectors, hypot, min, pointIsInCone, normalizeAngle
    ...
    def _resolveSpecifiers(cls, specifiers, defaults=None, overriding=False):
        ...
        if "On" in specifier_names:
            if collections.Counter(specifier_names)["On"] > 1:
                raise RuntimeParseError(
                    f'Cannot use "On" specifier to modify "On" specifier'
                )
        ...
        def dfs(spec):
            if spec._dfs_state == 2:  # finished processing this specifier
                ...
                raise RuntimeParseError(f"specifier {spec.name} " "depends on itself")
            spec._dfs_state = 1
        ...
        for spec in specifiers:
            for prop in spec.priorities:
                ...
                if prop not in self._propertiesSet:
                    raise RuntimeParseError(
                        f'object has no property "{prop}" to override'
                    )
...
class Object(OrientedPoint):
    ...
    def show_3d(self, viewer, highlight=False):
        ...
        if self.viewAngles != (math.tau, math.pi) or self.visibleDistance != 50:
            ...
            edges = camera_pyramid_mesh.face_adjacency_edges[
                camera_pyramid_mesh.face_adjacency_angles > np.radians(0.1)
            ].copy()
    ...
    def show_2d(self, workspace, plt, highlight=False):
        ...
        corners = [
            workspace.scenicToSchematicCoords(corner) for corner in self.corners[:4]
        ]
        x, y = zip(*corners)
...
def canSee(
    ...
    if rayCount is None:
        ...
        if distanceScaling:
            ...
            rayCount = (
                rayCount[0] * viewAngles[0] * target_distance,
                rayCount[1] * viewAngles[1] * target_distance,
            )
    ...
    if isinstance(target, (Region, Object)):
        ...
        if target_crosses_ahead and target_crosses_behind:
            ...
            if (
                (np.max(spherical_angles[:, 0]) < -viewAngles[0] / 2)
                or (np.min(spherical_angles[:, 0]) > viewAngles[0] / 2)
                or (np.max(spherical_angles[:, 1]) < -viewAngles[1] / 2)
                or (np.min(spherical_angles[:, 1]) > viewAngles[1] / 2)
            ):
                return False
