...
class _Constructible(Samplable):
    ...
    def __init__(self, *args, _internal=False, **kwargs):
        ...
        for spec in specifiers:
            ...
            for opt in spec.optionals:
                if (
                    opt in defs
                ):  # do not apply optionals for properties this object lacks
        ...
        def dfs(spec):
            if spec in done:
                ...
                raise RuntimeParseError(
                    f"specifier for property {spec.property} " "depends on itself"
                )
            seen.add(spec)
    ...
    def sampleGiven(self, value):
        return self.withProperties(
            {prop: value[getattr(self, prop)] for prop in self.properties}
        )
...
class OrientedPoint(Point):
    ...
    mutator: PropertyDefault(
        {"headingStdDev"}, {"additive"}, lambda self: HeadingMutator(self.headingStdDev)
    )
...
class Object(OrientedPoint, _RotatedRectangle):
    ...
    def _specify(self, prop, value):
        ...
        if prop == "behavior":
            ...
            value = toType(
                value, veneer.Behavior, f'"behavior" of {self} not a behavior'
            )
