...
def matchPolygonalField(heading, position):
    ...
    """
    if (
        isMethodCall(heading, VectorField.__getitem__)
        and isinstance(heading.object, PolygonalVectorField)
        and heading.arguments == (position,)
    ):
        return heading.object, 0, 0
    elif isinstance(heading, OperatorDistribution) and heading.operator in (
        "__add__",
        "__radd__",
    ):
...
def pruneContainment(scenario, verbosity):
    ...
    for obj in scenario.objects:
        ...
        if (
            minRadius is not None
        ):  # if we can lower bound the radius, erode the container
            containerPoly = containerPoly.buffer(-minRadius)
        elif base is container:
            continue
        newBasePoly = (
            basePoly & containerPoly
        ...
        newBase = regions.regionFromShapelyObject(
            newBasePoly, orientation=base.orientation
        )
