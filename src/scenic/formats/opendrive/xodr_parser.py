...
class Junction:
    ...
    def add_connection(
        self, incoming_id, connecting_id, connecting_contact, lane_links
    ):
...
class Road:
    def __init__(self, name, id_, length, junction, drive_on_right=True):
        ...
        self.remappedStartLanes = (
            None  # hack for handling spurious initial lane sections
        )
        ...
        for road in self.roads.values():
            for piece in road.ref_line:
                ...
                for i in range(len(ref_points) - 1):
                    ...
                    tan_vec = (
                        ref_points[i + 1][0] - ref_points[i][0],
                    ...
                    for id_ in offsets.keys():
                        if road.get_lane(id_, ref_points[i][2]).type_ == "driving":
                            bounds_x.append(
                                ref_points[i][0] + normal_vec[0] * offsets[id_]
                            )
                            bounds_y.append(
                                ref_points[i][1] + normal_vec[1] * offsets[id_]
                            )
            ...
            for c in connections:
                if c.incoming_id == road_id:
                    self.road_links.append(
                        RoadLink(
                            road_id, c.connecting_id, contact, c.connecting_contact
                        )
                    )
